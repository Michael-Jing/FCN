...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    up_sample1 = tf.layers.conv2d_transpose(conv_1x1, num_classes, 4, 2,
                                        padding="same",
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    vgg_layer4_out_channel_combined = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding="same",
                                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    mixed1 = tf.add(up_sample1, vgg_layer4_out_channel_combined)

    up_sample2 = tf.layers.conv2d_transpose(mixed1, num_classes, 4, 2, padding='same',
                                            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=ground_truth,
                                                                                logits=logits))
